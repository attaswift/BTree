...
internal struct MapValue<_Key: Comparable, Value>: RBValue {
    typealias Key = _Key
    ...
    func compare(key: Key, children: StateAccessor<MapValue<Key, Value>>, insert: Bool) -> RBComparisonResult<Key> {
        if key == self.key {
            return .Found
        }
...
public struct MapIndex<Key: Comparable, Value>: BidirectionalIndexType {
    ...
    internal typealias Tree = RBTree<TreeValue>
...
public struct MapGenerator<Key: Comparable, Value>: GeneratorType {
    ...
    private typealias Tree = RBTree<TreeValue>
    ...
    public mutating func next() -> Element? {
        ...
        let mv = tree[index].payload.value
...
public struct Map<Key: Comparable, Value>: SortedAssociativeCollectionType {
    ...
    public subscript(index: Index) -> Element {
        let value = tree[index.index!].payload.value
    ...
    public subscript(key: Key) -> Value? {
        get {
            ...
            return tree[index].payload.value.value
        }
        set(newValue) {
            ...
            case (.Some(let i), .Some(let value)):
                tree[i].payload.value = MapValue(key: key, value: value)
            }
        }
    }
    ...
    public mutating func updateValue(value: Value, forKey key: Key) -> Value?
    {
        let (index, slot) = tree.insertionSlotFor(key)
        if let index = index {
            let old = tree[index].payload.value.value
            tree[index].payload.value = MapValue(key: key, value: value)
            return old
        }
        else {
            tree.insert(MapValue(key: key, value: value), into: slot)
    ...
    public mutating func removeAtIndex(index: Index) -> (Key, Value) {
        let mv = tree[index.index!].payload.value
