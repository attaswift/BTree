...
extension BTree: BidirectionalCollection {
    ...
    public func index(_ i: Index, offsetBy n: Int, limitedBy limit: Index) -> Index? {
        checkIndex(i)
        checkIndex(limit)
        let d = distance(from: i, to: limit)
        if d > 0 ? d < n : d > n {
            return nil
        }
        var result = i
...
extension BTree {
    ...
    public mutating func insert(_ element: Element, atOffset offset: Int) {
        ...
        makeUnique()
        var pos = count - offset
        var splinter: BTreeSplinter<Key, Value>?
        var element = element
        edit(
            descend: { node in
                let slot = node.slot(atOffset: node.count - pos)
...
extension BTree {
    ...
    public mutating func remove(atOffset offset: Int) -> Element {
        ...
        makeUnique()
        var pos = count - offset
        var matching: (node: Node, slot: Int)?
        var old: Element?
        edit(
            descend: { node in
                let slot = node.slot(atOffset: node.count - pos)
...
extension BTree {
    ...
    public func prefix(through stop: Index) -> BTree {
        return prefix(upTo: index(after: stop))
    }
    ...
    public func subtree(with range: Range<Index>) -> BTree<Key, Value> {
        ...
        precondition(start >= 0 && start <= end && end <= count)
        if start == end {
            return BTree(order: order)
        }
        if start == 0 {
