...
public func + <Summary: SummaryProtocol>(a: Summary?, b: Summary?) -> Summary {
    switch (a, b) {
    case (nil, nil): return Summary()
    case (nil, .Some(let b)): return b
    case (.Some(let a), nil): return a
    case (.Some(let a), .Some(let b)): return Summary(a, b)
    }
}
...
public func + <Summary: SummaryProtocol>(a: Summary.Item?, b: Summary?) -> Summary {
    ...
    case (nil, nil): return Summary()
    case (nil, .Some(let b)): return b
    case (.Some(let a), nil): return Summary(a)
    case (.Some(let a), .Some(let b)): return Summary(Summary(a), b)
    }
}
...
public func + <Summary: SummaryProtocol>(a: Summary?, b: Summary.Item?) -> Summary {
    switch (a, b) {
    case (nil, nil): return Summary()
    case (nil, .Some(let b)): return Summary(b)
    case (.Some(let a), nil): return a
    case (.Some(let a), .Some(let b)): return Summary(a, Summary(b))
    }
}
