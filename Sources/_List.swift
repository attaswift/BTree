...
internal struct ListValue<Element>: RBValue {
    typealias Key = Int
    typealias State = Int
    ...
    func compare(key: Key, children: StateAccessor<ListValue<Element>>, insert: Bool) -> RBComparisonResult<Int> {
...
public struct List<Element>: ArrayLikeCollectionType {
    ...
    public subscript(index: Index) -> Element {
        get {
            ...
            return tree[i].payload.value.element
        }
        set {
            let i = tree.find(index)!
            tree[i].payload.value.element = newValue
        }
    }
    ...
    public mutating func removeAtIndex(index: Int) -> Element {
        ...
        let result = tree[index].payload.value.element
        tree.remove(index)
        return result
    }
}
