...
func assertEqualElements<Element: Equatable, S1: SequenceType, S2: SequenceType where S1.Generator.Element == Element, S2.Generator.Element == Element>(a: S1, _ b: S2, file: StaticString = #file, line: UInt = #line) {
    let aa = Array(a)
    let ba = Array(b)
...
func assertEqualElements<T1: Equatable, T2: Equatable, S1: SequenceType, S2: SequenceType where S1.Generator.Element == (T1, T2), S2.Generator.Element == (T1, T2)>(a: S1, _ b: S2, file: StaticString = #file, line: UInt = #line) {
    let aa = Array(a)
    let ba = Array(b)
...
extension BTree {
    ...
    internal func assertKeysEqual<S: SequenceType where S.Generator.Element == Key>(s: S, file: StaticString = #file, line: UInt = #line) {
